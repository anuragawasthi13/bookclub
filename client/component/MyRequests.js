			...
			<div style = {style1}>
				...
				{this.props.requests.map((request,i)=> 
					<div key={i} style= {style6}>
						<div style = {style5}> 
							<span style={style3}>{request.book.title} requested from <Link to = {{ pathname: "/user/" + request.requested }}>{request.requested}</Link></span>
						</div>
						
						<div style = {style5}>	
							<small>Status</small><span style={{marginLeft: "10px"}}>{request.granted ? "Accepted" : "Not Accepted"}</span>
						</div>
						{!request.granted && <button style={style4}>Cancel</button>}
